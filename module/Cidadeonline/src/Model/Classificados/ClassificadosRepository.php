...
class ClassificadosRepository extends AbstractRepository
{
     ...
     * @return \Cidadeonline\Model\Classificados\ClassificadosRepository
     */
    ...
    public function classificadosViews($url)
    {
        $data['classifcados_views'] = 0;
        $exist=$this->findBy(['url'=>$url]);

        if($exist->getResult())
        {
            $classifcados_views=$exist->getData()->current();
            $data['classifcados_views'] =((int)$classifcados_views->getClassifcadosViews()+1);
            return $this->tableGateway->update($data, array('id' => $classifcados_views->getId()));
        }
